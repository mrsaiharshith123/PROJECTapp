...
export default function ToolWidget({ icon, title, subtitle, accent, onClick }) {
  const accentClass =
    accent === "teal"
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group relative flex flex-col items-center justify-center text-center min-h-[9.5rem] p-4 rounded-2xl border bg-gradient-to-br ${accentClass} bg-white dark:bg-slate-900 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 active:translate-y-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500`}
    >
      <span
